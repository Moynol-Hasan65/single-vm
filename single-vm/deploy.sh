...
if [[ ! -f nginx/certs/server.crt || ! -f nginx/certs/server.key ]]; then
    echo "No TLS cert found at nginx/certs/ — generating a self-signed one for now."
    echo "  Replace with a real cert (e.g. Let's Encrypt) before going to production."
    mkdir -p nginx/certs
    openssl req -x509 -nodes -days 825 -newkey rsa:2048 \
        -keyout nginx/certs/server.key -out nginx/certs/server.crt \
        -subj "/CN=${HOST_IP:-localhost}" >/dev/null 2>&1
...
echo "  Web App:        https://${HOST_IP:-localhost}:3000"
echo "  Gophish Phish:  https://${HOST_IP:-localhost}:3000/landing"
